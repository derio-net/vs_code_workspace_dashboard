...
The system SHALL optionally support system tray integration for background operation.

#### Scenario: Tray icon is displayed
- **WHEN** the application is running
- **THEN** a tray icon SHALL be visible in the system tray

#### Scenario: Tray menu provides quick actions
- **WHEN** the user right-clicks the tray icon
- **THEN** a context menu SHALL appear with options: Show Dashboard, Check for Updates, Quit
